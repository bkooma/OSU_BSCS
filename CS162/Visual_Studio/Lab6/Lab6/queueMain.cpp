...
int main()
{
	...
	std::cout << "Welcome to the Doubly-Linked Circular Structure Queuing System\n";
	std::cout << "Please select an option from the following menu\n";
	...
	while (menuChoice != 5)
	{
		if (menuChoice == 1){
			...
			// If Queue is not empty, return the first node (front) value
				...
				std::cout << "The first node (front) value is: " << returnVal << std::endl << std::endl;
			}
		}
